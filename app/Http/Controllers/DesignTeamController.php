...
class DesignTeamController extends Controller
{
    public function index()
    {
        ...
        foreach ($teamDetails as $item) {
            ...
            $imagePath = "uploads/design_team/" . $image;
    ...
    public function add(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            if ($validator->fails())
            {
                ...
                $existingRecord = DesignTeam::orderBy('id','DESC')->first();
                $recordId = $existingRecord ? $existingRecord->id + 1 : 1;

                $img_path = $request->photo;
                $folderPath = "uploads/design_team/";
    ...
    {
        $all_data=[];
        $adminTeam = DesignTeam::find($id);
        $destination = 'uploads/design_team/'.$adminTeam->photo;
